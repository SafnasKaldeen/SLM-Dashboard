...
interface OverviewFilterProps {
  onFiltersChange?: (filters: OverviewFilter) => void;
...
export function OverviewFilter({
  ...
  // API-dependent filters (affect data fetching)
  const [filters, setFilters] = useState<OverviewFilter>({
  ...
  // Display-only filters (do not affect data fetching)
  ...
  // Effect for API-dependent filters
  useEffect(() => {
    onFiltersChange?.(filters);
  }, [filters, onFiltersChange]);

  // Effect for display-only filters
  useEffect(() => {
    onDisplayFiltersChange?.(displayFilters);
  }, [displayFilters, onDisplayFiltersChange]);

  const updateFilters = (newFilters: Partial<OverviewFilter>) => {
    const updated = { ...filters, ...newFilters };
    setFilters(updated);
  };

  const updateDisplayFilters = (newDisplayFilters: Partial<DisplayFilters>) => {
    const updated = { ...displayFilters, ...newDisplayFilters };
  ...
  const clearAllFilters = () => {
    ...
    const clearedDisplay: DisplayFilters = {
      vehicleIdSearch: "",
      vinSearch: "",
    };

    setFilters(cleared);
    setDisplayFilters(clearedDisplay);
  ...
  const getActiveFiltersCount = () => {
    ...
    // Include display filters in count
    if (displayFilters.vehicleIdSearch) count++;
    if (displayFilters.vinSearch) count++;
    return count;
  };
  ...
  const handleArrayFilterChange = (
    filterKey: keyof OverviewFilter,
    value: string,
    checked: boolean
  ) => {
    ...
    <Card>
      <CardContent className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          ...
          {/* Vehicle ID Search - Display Only */}
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                value={displayFilters.vehicleIdSearch}
                onChange={(e) =>
                  updateDisplayFilters({ vehicleIdSearch: e.target.value })
                }
                className="pl-10"
              />
              {displayFilters.vehicleIdSearch && (
                <Button
                  onClick={() => updateDisplayFilters({ vehicleIdSearch: "" })}
                  variant="ghost"
                  size="sm"
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                value={displayFilters.vinSearch}
                onChange={(e) =>
                  updateDisplayFilters({ vinSearch: e.target.value })
                }
                className="pl-10"
              />
              {displayFilters.vinSearch && (
                <Button
                  onClick={() => updateDisplayFilters({ vinSearch: "" })}
                  variant="ghost"
                  size="sm"
                  className="absolute right-2 top-1/2 transform -translate-y-1/2 h-6 w-6 p-0"
                >
