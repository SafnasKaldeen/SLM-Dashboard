...
import { MapPin, Navigation, Info, AlertTriangle, CheckCircle, Menu, X, Loader2, Zap } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { Card, CardContent } from "@/components/ui/card"
import dynamic from "next/dynamic"
import type { CanvasElement, DataSource } from "../../app/analytics/reports/types/professional-types"
...
const computeAverageCenter = (points: typeof mockMapData) => {
  ...
  const avgLng = longitudes.reduce((sum, val) => sum + val, 0) / longitudes.length
...
function LeafletMapComponent({ element, dataSources = [] }: CanvasMapProps) {
  ...
  const mapProvider = cfg.mapProvider ?? "cartodb_dark"
  ...
  const sizeField = cfg.sizeBy
  const colorField = cfg.colorBy
  const pingSpeedField = cfg.pingSpeedField
  ...
  useEffect(() => {
    ...
    mockMapData.forEach((point) => {
      if (point.type === "station") {
        stats.stations++
        if (point.status === "active") stats.safe++
  ...
  const getDataPoints = () => {
    ...
    return mockMapData.map((point) => ({
      ...point,
      lat: (point[latitudeField.id as keyof typeof point] as number) || point.latitude,
      lng: (point[longitudeField.id as keyof typeof point] as number) || point.longitude,
      size: sizeField ? (point[sizeField.id as keyof typeof point] as number) : point.revenue,
      color: colorField ? (point[colorField.id as keyof typeof point] as string) : point.status,
      pingSpeed: pingSpeedField ? (point[pingSpeedField.id as keyof typeof point] as number) : point.ping_speed,
    }))
  }
  ...
  // Get marker size based on size field
  const getMarkerSize = (point: any) => {
    if (!sizeField) return 32

    const value = point.size || 0
    const maxValue = Math.max(...dataPoints.map((p) => p.size || 0))
    const minValue = Math.min(...dataPoints.map((p) => p.size || 0))

    // Scale between 20 and 50 pixels
    const normalizedSize = ((value - minValue) / (maxValue - minValue)) * 30 + 20
    return Math.max(20, Math.min(50, normalizedSize))
  }

  // Get ping animation speed
  const getPingSpeed = (point: any) => {
    if (!pingSpeedField) return 1.5

    const speed = point.pingSpeed || 50
    // Lower ping = faster animation (better connection)
    // Scale between 0.8s (fast) and 3s (slow)
    return Math.max(0.8, Math.min(3, speed / 30))
  }
        ...
        <div style="font-size: 12px; line-height: 1.4;">
          <div style="margin-bottom: 4px;">
            ...
            <span style="font-weight: 500; margin-left: 4px;">
              ${(point.lat || point.latitude).toFixed(4)}, ${(point.lng || point.longitude).toFixed(4)}
            </span>
          </div>
          ...
          ${
            ...
            <div style="margin-bottom: 4px;">
              ...
              <span style="font-weight: 500; margin-left: 4px;">${point.lat || point.latitude}</span>
            </div>
          `
              : ""
          }
          ...
          ${
            ...
            <div style="margin-bottom: 4px;">
              ...
              <span style="font-weight: 500; margin-left: 4px;">${point.lng || point.longitude}</span>
            </div>
          `
              : ""
          }
          ...
          ${
            pingSpeedField
              ? `
            <div style="margin-bottom: 4px;">
              <span style="color: #94a3b8;">${pingSpeedField.name}:</span>
              <span style="font-weight: 500; margin-left: 4px;">${point.pingSpeed}ms</span>
            </div>
          `
              : ""
          }
          ...
          ${
            ...
            <div style="margin-bottom: 4px;">
              ...
              <span style="font-weight: 500; margin-left: 4px;">${point.utilization_rate}%</span>
            </div>
          `
              : `
    ...
    const initMap = () => {
      try {
        ...
        if (!mapInstance.current) {
          ...
          const currentProvider = mapProviders[mapProvider as keyof typeof mapProviders]
          ...
          L.tileLayer(currentProvider.url, tileOptions).addTo(mapInstance.current)
        ...
        .custom-marker-pulse {
          animation: pulse var(--ping-speed, 1.5s) infinite;
        }
        @keyframes pulse {
      ...
      `
        ...
        dataPoints.forEach((point) => {
          ...
          const pingSpeed = getPingSpeed(point)

          const customIcon = L.divIcon({
            className: "custom-marker-icon",
            html: `
          <div style="width: ${markerSize}px; height: ${markerSize}px; background-color: ${color}; display: flex; align-items: center; justify-content: center; border-radius: 50%; box-shadow: 0 0 15px rgba(0, 0, 0, 0.6); border: 2px solid rgba(255, 255, 255, 0.8);">
            <svg xmlns="http://www.w3.org/2000/svg" width="${Math.max(12, markerSize * 0.4)}" height="${Math.max(12, markerSize * 0.4)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ${iconSvg}
            </svg>
          </div>
          <div class="custom-marker-pulse" style="position: absolute; top: -${markerSize * 0.25}px; left: -${markerSize * 0.25}px; width: ${markerSize * 1.5}px; height: ${markerSize * 1.5}px; background-color: ${color}; opacity: 0.3; border-radius: 50%; --ping-speed: ${pingSpeed}s;"></div>
        `,
  ...
  }, [leaflet, mapProvider, dataPoints, latitudeField, longitudeField, sizeField, colorField, pingSpeedField])
      ...
      <>
        ...
        <div className="absolute top-4 right-4 z-[1000]">
          <Button
            ...
            variant="outline"
          >
            {uiCollapsed ? <Menu className="h-5 w-5 text-gray-700" /> : <X className="h-5 w-5 text-gray-700" />}
          </Button>
        </div>
                ...
                <CardContent className="p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-bold text-green-600">{mapStats.safe}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-bold text-yellow-600">{mapStats.warning}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-bold text-red-600">{mapStats.danger}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-bold text-blue-600">{mapStats.stations}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            {showLegend && (
              <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-lg border p-4 max-w-xs">
                ...
                <div className="space-y-3">
                  {/* Field Mappings */}
                  {(latitudeField || longitudeField || sizeField || colorField || pingSpeedField) && (
                    <div>
                      <div className="text-xs font-medium text-gray-700 mb-2">Field Mappings</div>
                      <div className="space-y-1 text-xs">
                        ...
                        {pingSpeedField && (
                          <div className="flex items-center gap-2">
                            ...
                            <span>Ping: {pingSpeedField.name}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {sizeField && (
                    <div>
                      <div className="text-xs font-medium text-gray-700 mb-2">Marker Size</div>
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <span>Based on {sizeField.name}</span>
                      </div>
                    </div>
                  )}
                  ...
                  {pingSpeedField && (
                    <div>
                      <div className="text-xs font-medium text-gray-700 mb-2">Ping Animation</div>
                      <div className="text-xs text-gray-600">Faster pulse = Lower ping (better connection)</div>
                    </div>
                  )}
                </div>
              </div>
    ...
    <div className="h-full w-full relative bg-gray-100 rounded-lg overflow-hidden">
      ...
      <div className="absolute top-2 left-2 z-[999]">
        <Badge variant="outline" className="bg-white/90 backdrop-blur-sm">
          {mapProviders[mapProvider as keyof typeof mapProviders]?.name || "OpenStreetMap"} (Z: {zoom})
        </Badge>
      </div>
      ...
      {(!latitudeField || !longitudeField) && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/80 backdrop-blur-sm z-[1002]">
          <div className="text-center text-gray-500 p-6">
            ...
            <div className="text-sm mb-4">Configure latitude and longitude fields to display the map visualization</div>
            <div className="space-y-2 text-xs text-left bg-gray-50 p-3 rounded-lg">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Optional: Size field (marker size)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-orange-500 rounded-full"></div>
                <span>Optional: Color field (marker color)</span>
              </div>
              <div className="flex items-center gap-2">
                <Zap className="w-2 h-2 text-cyan-500" />
                <span>Optional: Ping speed field (animation speed)</span>
              </div>
            </div>
          </div>
        </div>
